...
function checkTableFixed() {
    if ($('.hover-content').hasClass('wrapper')) {
        ...
        table_fixed_height = content_insider_height - content_header_height - content_footer_height - content_body_title_height - 100;
...
function removeItemNull(jsonObj) {
    for (var i = 0; i < jsonObj.length; i++) {
        if (jsonObj[i]["nodes"] != undefined && jsonObj[i]["nodes"] != "") {
            removeItemNull(jsonObj[i]["nodes"]);
        }
        else {
            delete (jsonObj[i]["nodes"]);
        }
    }
    jsonObj = jsonObj[i];
}