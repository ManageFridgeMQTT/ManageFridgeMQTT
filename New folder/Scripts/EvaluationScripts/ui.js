...
function checkTableFixed() {
    if ($('.table-box').hasClass('wrapper')) {
        ...
        table_fixed_height = content_insider_height - content_header_height - content_footer_height - 20;
    ...
    /*
	if (!$('.pane>div').hasClass('tablesorter-scroller')) {
		$('.tab-content .tablesorter').tablesorter({
			headerTemplate : '{content} {icon}', // Add icon for various themes
			widgets: [ 'uitheme', 'zebra', 'stickyHeaders', 'filter', 'scroller' ],
			widgetOptions: {
				stickyHeaders_attachTo : '.wrapper', // or $('.wrapper')
...
function checkScreen() {
    ...
    if (!$('html').hasClass('nav-mini') && screen_width <= 960) {
        $('html').addClass('nav-mini');
    }
    if ($('html').hasClass('nav-mini') && screen_width > 960) {
        //$('html').removeClass('nav-mini');
